<div>
    ...
    <a href="javascript:void(0)" id="modal-tenant-create-open" hidden data-bs-toggle="modal"
        data-bs-target="#modal-tenant-create" class="btn btn-primary"></a>

    <div class="modal modal-blur fade" id="modal-tenant-create" tabindex="-1" aria-hidden="true" style="display: none;"
        wire:ignore.self>
        ...
        <div class="modal-dialog modal-lg modal-dialog-centered" role="document">
            ...
            <form wire:loading.attr="disabled" wire:submit.prevent="store" class="modal-content">
                    ...
                    <div class="row">
                        ...
                        <div class="mb-3 col-6 @error('tenant.relocation_type') is-invalid @enderror">
                            <div wire:ignore>
                                ...
                                <div id="selectize-tenant-relocation_type_create-wrapper">
                                </div>
                                <script>
                                    function selectizeTenantRelocationTypeCreate() {

                                        $("#selectize-tenant-relocation_type_create-wrapper").empty();

                                        $('<select id="selectize-tenant-relocation_type_create" class="form-control"></select>')
                                            .appendTo('#selectize-tenant-relocation_type_create-wrapper');

                                        $("#selectize-tenant-relocation_type_create")
                                        ...
                                        $('#selectize-tenant-relocation_type_create').selectize({
                                            persist: true
                                        });

                                        $('#selectize-tenant-relocation_type_create').on('change', () => {
                                            let val = $('#selectize-tenant-relocation_type_create')
                                                .val();
                ...
                <div class="modal-footer">
                    ...
                    <button type="button" id="modal-tenant-create-dismiss" class="btn btn-link link-secondary"
    ...
    <script>
        document.addEventListener('livewire:load', function() {
            $('#modal-tenant-create').modal({backdrop:'static', keyboard:false});
            window.livewire.on('showTenantCreateForm', function() {
                selectizeTenantRelocationTypeCreate();
                document.getElementById('modal-tenant-create-open').click();
            });
            window.livewire.on('closeTenantCreateForm', function() {
                document.getElementById('modal-tenant-create-dismiss').click();
            });
        });
